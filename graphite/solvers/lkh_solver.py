...
class LKHGeneticSolver(BaseSolver):
    ...
    async def solve(self, formatted_problem: List[List[Union[int, float]]], future_id: int) -> List[int]:
        ...
        for run in range(self.runs):
            ...
            # Use Lin-Kernighan heuristic to improve the initial tour
            for idx in range(len(self.population)):
                tour, _ = self.find_tour(self.population[idx][0])
                cost = self.calculate_cost(tour)
                self.population[idx] = (tour, cost)
    ...
    def find_tour(self, tour: List[int]) -> Tuple[List[int], float]:
        # Implement Lin-Kernighan heuristic here or use a library function if available
        return tour, self.calculate_cost(tour)  # Placeholder
