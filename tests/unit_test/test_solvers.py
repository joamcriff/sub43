...
class TestSolvers(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_solvers(self):
        ...
        solvers = [DPSolver(), NearestNeighbourSolver(), BeamSearchSolver(), LKHGeneticSolver()]
        ...
        general_synapses = {solver_type: GraphSynapse(problem=self.general_tsp, solution=solution) for solver_type, solution in general_solutions.items()}
        large_solutions = {solver.__class__.__name__: await solver.solve_problem(self.large_tsp) for solver in solvers}
        large_synapses = {solver_type: GraphSynapse(problem=self.large_tsp, solution=solution) for solver_type, solution in large_solutions.items()}
        metric_score_handler = ScoreResponse(GraphSynapse(problem=self.metric_tsp))
        await metric_score_handler.get_benchmark()
        general_score_handler = ScoreResponse(GraphSynapse(problem=self.general_tsp))
        await general_score_handler.get_benchmark()
        large_score_handler = ScoreResponse(GraphSynapse(problem=self.large_tsp))
        ...
        for compared_solver in [BeamSearchSolver, NearestNeighbourSolver(), LKHGeneticSolver()]:
